...
import { language } from "../Game/Language/index";
...
export const showTips = (tipId) => {
  ...
  let str = language[lang].tips[tipId];

  cc.resources.load('Tips', cc.Prefab, (error, prefab) => {

    if (error) {
      console.log('error');
      return;
    }
    // console.log(prefab, '动态加载的prefab');
    ...
    tips.scale = 0;
    tips.position = cc.v3(position.x, 480, 0);
