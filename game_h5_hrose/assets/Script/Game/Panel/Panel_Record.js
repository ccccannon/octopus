...
cc.Class({
    ...
    // 初始化列表
    initRocordList(info) {
       const item_horse_viewList = this.resNode.getComponent('resNodeCtrl').getHorseViewList();
        // console.log(info, '39');
        const resultNode = cc.instantiate(this.prefab_item_record_result);
        for (let i = 0, len = info.length; i < len; i++) {
            const item = cc.instantiate(this.prefab_item_record);
            const script = item.getComponent('item_panel_record');
            script.setResultItem(resultNode, info[i],item_horse_viewList);
            item.parent = this.recordContainer;
        }
    },
    ...
    async getRecordInfo(reqData) {
        ...
        const info = await GameData.getHorseRecordData(reqData);
