...
type ExecuterFunc = (callback: CallbackObject, msg: NetMsgBase) => void;
...
export class NetNode {
    ...
    protected _autoReconnect: number = -1;
    ...
    protected _heartTime: number = 6000;                                   // 心跳间隔
    ...
    public init(socket: ISocket, protocol: IProtocolHelper, networkTips: INetworkTips | null = null, execFunc: ExecuterFunc | null = null) {
        ...
        this._callbackExecuter = execFunc ? execFunc : (callback: CallbackObject, msg: NetMsgBase) => {
            callback.callback.call(callback.target, msg);
        }
    }
    ...
    public reConnect() {
        ...
        Logger.logBusiness(GameMgr.getInstance().GameId, '主动重连11111111111111111111111');
    ...
    protected onError(event: any) {
        Logger.logNet(`连接错误`);
        cc.systemEvent.emit('netOffline');
        cc.error(event);
    }
    ...
    protected onClosed(event: any) {
        ...
        Logger.logNet(`连接关闭`);
        // console.log(this._connectOptions);
        ...
        if (this.isAutoReconnect()) {
            ...
            this._reconnectTimer = setTimeout(() => {
                Logger.logBusiness(this._autoReconnect,'自动重连次数');
                this._socket!.close();
    ...
    public send(buf: NetData, force: boolean = false): number {
        if (this._state == NetNodeState.Working || force) {
            ...
            cc.error(`当前状态为【${NetNodeStateStrs[this._state]}】,请求错误`);
            return -1;
        }
    }
    ...
    /** 可添加多个同类返回消息的监听 */
    public addResponeHandler(cmd: any, callback: NetCallFunc, target?: any): boolean {
        if (callback == null) {
            cc.error(`命令为【${cmd}】添加响应处理程序错误`);
            return false;
        }
    ...
    /** 删除一个监听中指定子回调 */
    public removeResponeHandler(cmd: any, callback: NetCallFunc, target?: any) {
