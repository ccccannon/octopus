export class SoundManager {
    ...
    public static _instance: SoundManager = null;
    ...
    private effectCacheList = null;

    private bgCacheList = null;

    private playingList = null;
    ...
    loadAudioResource(path, type) {
        cc.resources.loadDir(path, cc.AudioClip, (err, audios) => {
            ...
            for (let i = 0, len = audios.length; i < len; i++) {
                const item = audios[i];
                // 判断是否已经存在同名音效
                if (container.has(item.name)) {
                    continue;
                }
                container.set(item.name, item);
            }
    ...
    loadResourceInBundle(bundleName, path, type) {
        ...
        bundle.loadDir(path, cc.AudioClip, (err, audios) => {
    ...
    /** 金币音效 */
    coinEffect() {
        this.playEffect('effect_gold', false, 0.3);
    }
