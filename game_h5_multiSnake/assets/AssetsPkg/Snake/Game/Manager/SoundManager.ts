import { isIos } from "../../../../Script/Utils/App_connect";
...
export default class SoundManager {
    ...
    /** 在资源包中加载资源  */
    loadResourceInBundle(bundleName, path, type) {

        const bundle = cc.assetManager.getBundle(bundleName);

        bundle.loadDir(path, cc.AudioClip, (err, audios: Array<cc.AudioClip>) => {

            // console.log(audios, ' /** 加载的资源 */');
            if (err) {
                console.log(err);
                return;
            }
            ...
            // console.log(container, '在资源包中加载资源');
        })
    ...
    eatLittleFoodEffect() {
        ...
        // this.playEffect('effect_eat_small', false, 0.1);
        if (isIos()) {
            this.playEffect('effect_tap', false, 1);
        } else {
            this.playEffect('effect_tap', false, 0.3);
        }
    ...
    eatBigFoodEffect() {
        ...
        if (isIos()) {
            this.playEffect('effect_eat_big', false, 0.8);
        } else {
            this.playEffect('effect_eat_big', false, 0.2);
        }
    }
    ...
    deathEffect() {
        ...
        if (isIos()) {
            this.playEffect('effect_end', false, 1.5);
        } else {
            this.playEffect('effect_end', false, 0.3);
        }
