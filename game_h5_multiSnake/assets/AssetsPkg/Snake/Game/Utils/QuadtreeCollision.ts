...
export default class QuadtreeCollision {
    ...
    updateTree(containerNode) {
        ...
        const collider = containerNode.getComponentsInChildren(cc.Collider);

        // console.log(collider, '更新树结构');

        for (let i = 0, l = collider.length; i < l; i++) {
            const item: cc.Collider = collider[i];
            // 更新碰撞体世界aabb
            collisionManager['updateCollider'](item);
            const aabb = item['world'].aabb;
            const rect = { x: aabb.x, y: aabb.y, height: aabb.height, width: aabb.width, collider: item };
    ...
    check(testCollider: cc.Collider) {
        if (!this._tree) {
            return;
        }
