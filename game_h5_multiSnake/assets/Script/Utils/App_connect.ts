...
/**分享游戏 */
export const openShare = (content: string) => {

    if (CC_DEBUG) {
        console.log('分享游戏: openShare', content);
    }

    if (isIos()) {
        // @ts-ignore
        const handler = window.webkit.messageHandlers;
        if (handler.openShare) {
            handler.openShare.postMessage(content);
        } else {
            console.log('unexcept sutiation on openShare in ios')
        }
        return;
    }
    ...
    if (isAndroid()) {
        ...
        const handler = window.androidJsObj;
        if (handler.openShare) {
            handler.openShare(content);
        } else {
            console.log('unexcept sutiation on openShare in android ');
        }
...
/** 接收app分享的结果 */
export const onShareResponse = (callback) => {
    if (CC_DEBUG) {
        console.log('接收游戏分享的结果');
    }

    // @ts-ignore
    if (window && !window.onShareResponse) {
        if (CC_DEBUG) {
            console.log('接收游戏分享的结果: onShareResponse');
        }
        // @ts-ignore
        window.onShareResponse = (res: string) => {
            console.log(res, "接收游戏分享的结果");
            callback && callback(res);
        };
    } else {
        callback = null;
